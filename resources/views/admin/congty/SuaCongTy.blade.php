...
@section('content')
    ...
    <form class="user" action="admin/congty/sua/{{ $congty->id }}" method="post">
        <hr />
        <div class="form-group">
            <input type="hidden" name="_token" value="{{csrf_token()}}">
            <h6 class="col-sm-4">Tên công ty</h6>
            <input type="text" class="form-control form-control-user" name="ten_cong_ty"
                   value="{{$congty->ten_cong_ty}}">
        </div>
        <div class="form-group">
            <h6 class="col-sm-4">Nhập địa chỉ công ty</h6>
            <input type="text" class="form-control form-control-user"
                   placeholder="Nhập tên công ty mới" name="dia_chi_cong_ty"
                   value="{{$congty->dia_chi_cong_ty}}">
        </div>
        <div class="form-group">
